...
const ChatSend_Cust: React.FC = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Prepare payload: only id_scoot_send + teks_customer/teks_driver
      const payload: any = { id_scoot_send: orderId };
      ...
      // Use .insert().select() to get inserted row back
      const { data, error } = await supabase.from('chat_send').insert([payload]).select();
      if (error) throw error;

      // Add returned row(s) to local state so UI updates immediately
      if (data && data.length > 0) {
        ...
        if (row.teks_customer) {
          newMsgs.push({
            ...
            sender: 'customer',
            timestamp: new Date(row.timestamp).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', hour12: false })
          });
        }
        if (row.teks_driver) {
          newMsgs.push({
      ...
      // realtime subscription will also receive the insert (harmless duplicate)
    } catch (err) {
      console.error('Gagal mengirim pesan:', err);
      Alert.alert('Error', 'Gagal mengirim pesan');
      setInputText(text); // restore input for retry
    } finally {
      setIsSending(false);
    }
  };
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.content}>
          <View style={styles.header}>
            ...
            <View style={styles.headerTextContainer}><Text style={styles.counterpartName}>{counterpartInfo.nama}</Text><Text style={styles.statusBadge}>Scoot Send Order #{orderId}</Text></View>
          </View>
