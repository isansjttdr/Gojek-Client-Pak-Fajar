...
const ChatFood_Cust: React.FC = () => {
  ...
  const [orderDetail, setOrderDetail] = React.useState<OrderDetailFood | null>(null);
  ...
  React.useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_food', filter: `id_scoot_food=eq.${orderId}` }, (payload) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const payload: any = { id_scoot_food: orderId, timestamp: new Date().toISOString() };
      ...
      const { error } = await supabase.from('chat_food').insert([payload]);
      if (error) throw error;
    } catch (err) {
      console.error(err);
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.content}>
          <View style={styles.header}>
            ...
            <View style={styles.headerTextContainer}><Text style={styles.counterpartName}>{counterpartInfo.nama}</Text><Text style={styles.statusBadge}>Scoot Food Order #{orderId}</Text></View>
          </View>

          {orderDetail && (
            <View style={styles.orderCard}>
              <View style={styles.orderRow}><Text style={styles.orderLabel}>📍 Resto:</Text><Text style={styles.orderValue}>{orderDetail.lokasi_resto}</Text></View>
              ...
              <View style={styles.orderRow}><Text style={styles.orderLabel}>🍽️ Pesanan:</Text><Text style={styles.orderValue}>{orderDetail.detail_pesanan}</Text></View>
              <View style={styles.orderRow}><Text style={styles.orderLabel}>💰 Ongkir:</Text><Text style={styles.orderValue}>Rp {Number(orderDetail.ongkir).toLocaleString('id-ID')}</Text></View>
